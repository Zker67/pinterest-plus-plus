...
- 过滤 `其中包括图片：` 等 Pinterest 自动图片描述，避免无标题 Pin 被误命名为描述文案。
...
- 改为优先使用 blob 下载，确保浏览器按脚本生成的文件名保存。

## 0.8.29

- 下载文件名优先使用 Pin 标题；无标题时使用 `pinterest-{pinId}`。
- 修复候选 URL 下载时丢失 Pin 文件名前缀的问题。

## 0.8.28

- 复制按钮恢复为复制原始媒体 URL，避免浏览器图片剪贴板兼容性问题。
- 移除图片 blob 写入剪贴板的实验逻辑。
